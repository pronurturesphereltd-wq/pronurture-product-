...
- **Targeted-swap addendum: verified live.** Offering with no target is refused; Chidi — same facility, same `A&E Nurse` role, simply not the named colleague — could not see the offer in her list and got the byte-identical missing-request 404 when calling accept directly with the correct id.
- **Two honest caveats on that "verified", so nobody reads more into it than is there:**
  - The concurrency proof runs on **SQLite** in the default suite, and SQLite serialises writers. The genuine row-level race needs `PSL_TEST_ON_POSTGRES=1`, and the test was **rewritten** for targeting after the last Postgres run — so the current version has never been raced on Postgres. `test_database_backend_under_test` records the backend so a green run cannot be mistaken for the stronger guarantee.
  - The targeted-swap flow was proven **through the API**, end to end with real tokens. The two-step picker on `/me` has not yet been clicked through by a human. Everything before the addendum was clicked; this part is one step short of this project's usual bar.
