...
- **Phase 1B is complete.** All six done-criteria verified against live infrastructure, plus both addenda — the role guardrail and targeted-only swaps. 343 tests passing, `next build`, `eslint` and the OpenAPI schema clean.
  1. **Swap request and accept.** Bola offered shift 6 to Amaka by name; Amaka accepted; `Shift.professional` moved to her.
  2. **Two simultaneous accepts, exactly one wins.** Proven by racing real threads. See the caveat below — this one is test-only by nature.
  3. **The facility sees accepted swaps.** Its rota shows the new assignee and the swap panel lists the settled offer.
  4. **Leave submitted, approved and declined** through the web app, both branches, each transition recorded in history: Amaka's approved, Bola's declined.
  5. **Compliance.** Chidi's licence expiring 2026-10-04 — exactly on the 30-day boundary — produced one open alert, created by a **real scheduled run** (`colorado-red-queen-twelve`, executed by `qcluster`, `alerts_created: 1`), not an inline call. Repeat runs create nothing while one is open.
  6. **Facility isolation** across swap requests, leave applications and compliance alerts. This pass found two real leaks rather than confirming the code was fine — see the cross-tenant lookup section.
- **Targeted-swap addendum: verified at both levels.**
  - *Through the API:* offering with no target is refused; Chidi — same facility, same `A&E Nurse` role, simply not the named colleague — could not see the offer in her list and got the byte-identical missing-request 404 when calling accept directly with the correct id.
  - *Through the UI:* the picker listed both eligible colleagues, the non-target saw nothing, and the target accepted. Swap request 4 records it — Bola offered shift 6 to Chidi at 20:40, Chidi accepted at 20:42 — and the shift's assignee history reads `Amaka → Bola → Amaka → Bola → Chidi` across the whole session's runs.
- **One caveat remains on that "verified", so nobody reads more into it than is there.** The concurrency proof runs on **SQLite** in the default suite, and SQLite serialises writers. The genuine row-level race needs `PSL_TEST_ON_POSTGRES=1`, and the test was **rewritten** for targeting after the last Postgres run — so the current version has never been raced on Postgres. `test_database_backend_under_test` records the backend so a green run cannot be mistaken for the stronger guarantee. This is the one done-criterion that cannot be settled by clicking; it needs a Postgres run, ideally against a local instance rather than Supabase.
- **Next work is Phase 1B's follow-ups or Phase 2 (Jobs). Neither is started.**
- **The isolation pass was not just a review.** Every Phase 1B endpoint has tests against each hostile actor — foreign facility, foreign professional, wrong role, unapproved facility, and a professional with no facility at all. It found a real information leak and a second, subtler instance of it; see the cross-tenant lookup section.
- **Role guardrail on swaps** (PSL_Phase1B_Role_Guardrail_Addendum.md) and **targeted-only offers** (PSL_Phase1B_Targeted_Swap_Addendum.md) each have their own section below.
...
- **Not yet built:** Phase 2 (Jobs), Phase 3 (Academy), Phase 4 (Analytics).
